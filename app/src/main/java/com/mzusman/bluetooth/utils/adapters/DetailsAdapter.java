...
public class DetailsAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(name.getText().equals(AvailableCommandNames.ENGINE_RPM.getValue()) && Integer.valueOf(value.getText().toString()) > 1000){
            Toast.makeText(context,"RPM is too high",Toast.LENGTH_LONG).show();
        }
        if(name.getText().equals(AvailableCommandNames.SPEED.getValue()) && Integer.valueOf(value.getText().toString()) > 5){
            Toast.makeText(context,"Speed is too high",Toast.LENGTH_LONG).show();
